...
class OpeningScreen(Image, ButtonBehavior):
    pass
...
_DEFAULT_TEXT_LABEL = Label(text="{}%", font_size=0)#hehe
...
class GUI(App):
    ...
    def cycle(self, readCYCLETIME):
        #init GUI variable (sliders, )
        self.manLF = self.root.ids.manLF.value
        self.manRF = self.root.ids.manRF.value
        self.manLB = self.root.ids.manLB.value
        self.manRB = self.root.ids.manRB.value
        ...
        #write HW variabels to HW (output, )
        ...
        #set GUI variabels (circular progressbar,)
        self.root.ids.LFoutGui.value = int(self.LFout)
        self.root.ids.RFoutGui.value = int(self.RFout)
        self.root.ids.LBoutGui.value = int(self.LBout)
        self.root.ids.RBoutGui.value = int(self.RBout)

    #setts manual thurst / slider value to zero
    def turnOffThruster(self, id):
        if id == "LF":
            self.root.ids.manLF.value = 0
        elif id == "RF":
            self.root.ids.manRF.value = 0
        elif id == "LB":
            self.root.ids.manLB.value = 0
        elif id == "RB":
            self.root.ids.manRB.value = 0
